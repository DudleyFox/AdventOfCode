...
if __name__ == '__main__':
    ...
    currentFree = availableSpace - usedSpace

    sizes = getSizes(root)

    candidate = availableSpace
    for s in sizes:
        if currentFree + s > desiredSpace:
            candidate = min(s,candidate)

    print(f'Available: {availableSpace}')
    print(f'UsedSpace: {usedSpace}')
    print(f'Delete {candidate} for {usedSpace+candidate} total free space')
