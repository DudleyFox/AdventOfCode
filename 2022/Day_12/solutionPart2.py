...
def getShortestPath(points,topo):
    maxX = len(topo[0])
    maxY = len(topo)
    visited = set()
    for x,y in points:
        visited.add((x,y))
    pathsTaken = [[p] for p in points]
    return buildPaths(pathsTaken, maxX, maxY, topo, visited)

if __name__ == '__main__':
    topo = readMap()
    points = getStartingPoints(topo)
    distance = getShortestPath(points,topo)
    print(distance)
